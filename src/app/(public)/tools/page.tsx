    ...
    <main className="mx-auto max-w-[var(--max-width)] px-6 pb-20 pt-12">
      <div className="mb-10">
        ...
        <p className="max-w-[560px] text-base leading-[1.65] text-text-secondary">
          Independent reviews and comparisons of the tools that power modern
          outbound. No affiliate bias — just what works.
        </p>
      </div>
      ...
      <div className="mb-10 flex flex-wrap gap-2">
        <Link
          href="/tools"
          className="rounded-full bg-text-primary px-4 py-1.5 text-xs font-semibold text-white no-underline"
        >
          All Tools
        </Link>
        {categories.map((cat) => (
          <Link
            ...
            className="rounded-full border border-border bg-surface px-4 py-1.5 text-xs font-medium text-text-secondary no-underline transition-colors hover:border-border-hover hover:text-text-primary"
          >
            {cat.name}
          </Link>
        ))}
